...
Door::Door( vec3 pos, int num ) {
	...
	switch (n) {
	case 1:
		modelmatrix = Translate(pos.x, pos.y, pos.z)*RotateY(180);
		break;
	case 2:
	...
	vec4 v[] = { vec4(-3.0,-0.4,0.4,1.0),vec4(-3.0,4.0,0.4,1.0),vec4(2.0,4.0,0.4,1.0),vec4(2.0,-0.4,0.4,1.0), vec4(-3.0,-0.4,-0.4,1.0),vec4(-3.0,4.0,-0.4,1.0),vec4(2.0,4.0,-0.4,1.0),vec4(2.0,-0.4,-0.4,1.0) };
	setVertices(v);
	buildCube();
	setTexture("door.ppm");
...
void Door::makeQuad(int a, int b, int c, int d) {
...
void Door::draw(Camera* c, vector<Light> lights) {
...
void Door::draw(Camera* c, GLuint p) {
	glUseProgram(program);
	glBindVertexArray(VAO);
...
int Door::testColision(vec4 ray, Camera* c) {
	vec4 v1, v2, v3;
	int closestT = -1;
	int ind = -1;
	bool inside;
...
void Door::setMaterial(vec4 d, vec4 a, vec4 s, float sh) {
	matDiffuse = d;
	matAmbient = a;
	matSpecular = s;
	shininess = sh;
...
void Door::shrink() {
	modelmatrix = modelmatrix*Scale(0.9);
}
